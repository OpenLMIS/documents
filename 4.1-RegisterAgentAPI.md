...
**Description**: The "Active" flag in API will accept only two values : {True | False}, being an optional field in API it can be kept null. However, OpenLMIS will check for valid values if it is not kept null.
#### 5) *Malformed JSON*
**Response**:  
{        
   "error": "Invalid JSON"      
